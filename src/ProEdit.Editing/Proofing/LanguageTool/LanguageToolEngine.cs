...
public sealed class LanguageToolEngine : IGrammarEngine, IStyleEngine
{
    private readonly IProofingEngineHost _host;
    private readonly LanguageToolEngineOptions _options;

    public string EngineId => _host.EngineId;

    public LanguageToolEngine(IProofingEngineHost host, LanguageToolEngineOptions? options = null)
    {
        _host = host ?? throw new ArgumentNullException(nameof(host));
        _options = options ?? new LanguageToolEngineOptions();
    }
    ...
    private bool ShouldIncludeKind(ProofingIssueKind kind)
    {
        return kind switch
        {
            ...
            ProofingIssueKind.Style => _options.IncludeStyle,
            _ => false
        };
    }
