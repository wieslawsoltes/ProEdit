...
public sealed class FlowDocumentConverter
{
    ...
    /// <summary>
    /// Converts the specified <see cref="ProEdit.FlowDocument.FlowDocument"/> into a <see cref="Document"/>.
    /// </summary>
    /// <param name="source">The FlowDocument to convert.</param>
    /// <returns>The converted document.</returns>
    public Document Convert(ProEdit.FlowDocument.FlowDocument source)
    {
    ...
    private void ApplyDocumentDefaults(ProEdit.FlowDocument.FlowDocument source, Document target)
    {
    ...
    private void AppendBlock(ProEdit.FlowDocument.Block block, IList<DocumentBlock> target)
    {
        switch (block)
        {
            ...
            case ProEdit.FlowDocument.Table table:
                ...
                break;
            case ProEdit.FlowDocument.BlockUIContainer blockUi:
    ...
    private void AppendList(ProEdit.FlowDocument.List list, IList<DocumentBlock> target, int level)
    {
    ...
    private void AppendBlockWithListInfo(
        ProEdit.FlowDocument.Block block,
        IList<DocumentBlock> target,
        ProEdit.FlowDocument.List list,
        int listId,
        int level,
        ref bool startApplied)
    {
        switch (block)
        {
            ...
            case ProEdit.FlowDocument.Section section:
                ...
                break;
            case ProEdit.FlowDocument.List nestedList:
    ...
    private ParagraphBlock ConvertParagraph(ProEdit.FlowDocument.Paragraph paragraph, ListInfo? listInfo)
    {
        ...
        var baseStyle = paragraph.Style.HasValues ? paragraph.Style.Clone() : new ProEdit.FlowDocument.FlowTextStyleProperties();
    ...
    private void ApplyParagraphProperties(ProEdit.FlowDocument.Block source, ParagraphBlock target)
    {
        if (source.TextAlignment.HasValue)
        {
        ...
        if (source is ProEdit.FlowDocument.Paragraph paragraph && paragraph.TextIndent.HasValue)
        {
            target.Properties.FirstLineIndent = (float)paragraph.TextIndent.Value;
        }

        if (source is ProEdit.FlowDocument.Paragraph flowParagraph)
        {
    ...
    private void AppendInlines(
        ProEdit.FlowDocument.InlineCollection inlines,
        ParagraphBlock paragraph,
        ProEdit.FlowDocument.FlowTextStyleProperties currentStyle,
    ...
    private void AppendInline(
        ProEdit.FlowDocument.Inline inline,
        ParagraphBlock paragraph,
        ProEdit.FlowDocument.FlowTextStyleProperties currentStyle,
        HyperlinkInfo? hyperlink)
    {
        if (inline is ProEdit.FlowDocument.Run run)
        {
            AppendRun(run.Text, paragraph, currentStyle, hyperlink);
            return;
        }

        if (inline is ProEdit.FlowDocument.LineBreak)
        {
            AppendRun("\n", paragraph, currentStyle, hyperlink);
            return;
        }

        if (inline is ProEdit.FlowDocument.InlineUIContainer inlineUi)
        {
        ...
        if (inline is ProEdit.FlowDocument.AnchoredBlock anchored)
        {
            paragraph.FloatingObjects.Add(ConvertAnchoredBlock(anchored));
            return;
        }

        if (inline is ProEdit.FlowDocument.Span span)
        {
            ...
            if (span is ProEdit.FlowDocument.Hyperlink link)
            {
    ...
    private void AppendRun(
        string? text,
        ParagraphBlock paragraph,
        ProEdit.FlowDocument.FlowTextStyleProperties style,
        HyperlinkInfo? hyperlink)
    {
    ...
    private TableBlock ConvertTable(ProEdit.FlowDocument.Table table)
    {
    ...
    private ParagraphBlock ConvertBlockUiContainer(ProEdit.FlowDocument.BlockUIContainer container)
    {
    ...
    private FloatingObject ConvertAnchoredBlock(ProEdit.FlowDocument.AnchoredBlock anchored)
    {
        var alignment = anchored is ProEdit.FlowDocument.Floater
            ? FloatingHorizontalAlignment.Right
            : FloatingHorizontalAlignment.Left;
        var width = anchored switch
        {
            ProEdit.FlowDocument.Figure figure when figure.Width.HasValue => (float)Math.Max(1d, figure.Width.Value),
            ProEdit.FlowDocument.Floater floater when floater.Width.HasValue => (float)Math.Max(1d, floater.Width.Value),
            _ => 200f
        };
        var height = anchored is ProEdit.FlowDocument.Figure withHeight && withHeight.Height.HasValue
    ...
    private bool TryCreateAnchoredEmbeddedUiShape(
        ProEdit.FlowDocument.AnchoredBlock anchored,
        out ShapeInline shape)
    {
        ...
        if (anchored.Blocks[0] is ProEdit.FlowDocument.BlockUIContainer blockUi)
        {
            return TryCreateEmbeddedUiShape(blockUi.Child, isInline: false, out shape);
        }

        if (anchored.Blocks[0] is ProEdit.FlowDocument.Paragraph paragraph
            && paragraph.Inlines.Count == 1
            && paragraph.Inlines[0] is ProEdit.FlowDocument.InlineUIContainer inlineUi)
        {
    ...
    private ShapeTextBox BuildShapeTextBox(ProEdit.FlowDocument.AnchoredBlock anchored)
    {
    ...
    private DocumentTableCell ConvertTableCell(
        ProEdit.FlowDocument.TableCell cell,
        int columnSpan,
        int rowSpan,
        int columnIndex)
    {
    ...
    private static void ApplyFlowTableCellVisualProperties(
        ProEdit.FlowDocument.TableCell source,
    ...
    private static void ApplyFlowBordersToParagraph(
        ProEdit.FlowDocument.FlowThickness thickness,
        string? brush,
    ...
    private static ListInfo CreateListInfo(
        ProEdit.FlowDocument.FlowListMarkerStyle style,
        int level,
        int listId,
        int? startIndex,
        ref bool startApplied)
    {
        ...
        if (style == ProEdit.FlowDocument.FlowListMarkerStyle.Disc)
        {
            info.NumberFormat = ListNumberFormat.Bullet;
            info.BulletSymbol = "•";
        }
        else if (style == ProEdit.FlowDocument.FlowListMarkerStyle.Circle)
        {
            info.NumberFormat = ListNumberFormat.Bullet;
            info.BulletSymbol = "○";
        }
        else if (style == ProEdit.FlowDocument.FlowListMarkerStyle.Square)
        {
            info.NumberFormat = ListNumberFormat.Bullet;
            info.BulletSymbol = "■";
        }
        else if (style == ProEdit.FlowDocument.FlowListMarkerStyle.LowerRoman)
        {
            info.NumberFormat = ListNumberFormat.LowerRoman;
        }
        else if (style == ProEdit.FlowDocument.FlowListMarkerStyle.UpperRoman)
        {
            info.NumberFormat = ListNumberFormat.UpperRoman;
        }
        else if (style == ProEdit.FlowDocument.FlowListMarkerStyle.LowerLatin)
        {
            info.NumberFormat = ListNumberFormat.LowerLetter;
        }
        else if (style == ProEdit.FlowDocument.FlowListMarkerStyle.UpperLatin)
        {
            info.NumberFormat = ListNumberFormat.UpperLetter;
        }
        else if (style == ProEdit.FlowDocument.FlowListMarkerStyle.Decimal)
        {
    ...
    private static ListKind ConvertListKind(ProEdit.FlowDocument.FlowListMarkerStyle style)
    {
        return style switch
        {
            ProEdit.FlowDocument.FlowListMarkerStyle.Decimal => ListKind.Numbered,
            ProEdit.FlowDocument.FlowListMarkerStyle.LowerLatin => ListKind.Numbered,
            ProEdit.FlowDocument.FlowListMarkerStyle.UpperLatin => ListKind.Numbered,
            ProEdit.FlowDocument.FlowListMarkerStyle.LowerRoman => ListKind.Numbered,
            ProEdit.FlowDocument.FlowListMarkerStyle.UpperRoman => ListKind.Numbered,
            ProEdit.FlowDocument.FlowListMarkerStyle.None => ListKind.None,
            _ => ListKind.Bullet
        };
    }

    private static ParagraphAlignment ConvertAlignment(ProEdit.FlowDocument.FlowTextAlignment alignment)
    {
        return alignment switch
        {
            ProEdit.FlowDocument.FlowTextAlignment.Center => ParagraphAlignment.Center,
            ProEdit.FlowDocument.FlowTextAlignment.Right => ParagraphAlignment.Right,
            ProEdit.FlowDocument.FlowTextAlignment.Justify => ParagraphAlignment.Justify,
            _ => ParagraphAlignment.Left
        };
    }

    private static HyperlinkInfo? BuildHyperlink(ProEdit.FlowDocument.Hyperlink link)
    {
        if (link is null)
        {
            return null;
    ...
    private static TextStyleProperties? ConvertTextStyle(ProEdit.FlowDocument.FlowTextStyleProperties style)
    {
        if (!style.HasValues)
        {
            return null;
    ...
    private static void ApplyTextStyle(ProEdit.FlowDocument.FlowTextStyleProperties style, TextStyle target)
    {
        ...
        {
            target.FontWeight = style.FontWeight.Value == ProEdit.FlowDocument.FlowFontWeight.Bold
                ? DocFontWeight.Bold
        ...
        {
            target.FontStyle = style.FontStyle.Value == ProEdit.FlowDocument.FlowFontStyle.Italic
                ? DocFontStyle.Italic
                : DocFontStyle.Normal;
        }
        ...
        {
            if (style.TextDecorations.Value.HasFlag(ProEdit.FlowDocument.FlowTextDecorations.Underline))
            {
                target.Underline = true;
                target.UnderlineStyle = DocUnderlineStyle.Single;
            }

            if (style.TextDecorations.Value.HasFlag(ProEdit.FlowDocument.FlowTextDecorations.Strikethrough))
            {
        ...
        if (style.BaselineAlignment.HasValue)
        {
            ...
            {
                ProEdit.FlowDocument.FlowBaselineAlignment.Superscript => DocVerticalPosition.Superscript,
                ProEdit.FlowDocument.FlowBaselineAlignment.Subscript => DocVerticalPosition.Subscript,
    ...
    private static void ApplyTextStyle(ProEdit.FlowDocument.FlowTextStyleProperties style, TextStyleProperties target)
    {
        if (style.FontFamily is not null)
        {
            target.FontFamily = style.FontFamily;
    ...
    private static ProEdit.FlowDocument.FlowDocument? FindDocument(ProEdit.FlowDocument.FlowElement element)
    {
        var current = element.Parent;
        while (current is not null)
        {
            if (current is ProEdit.FlowDocument.FlowDocument document)
            {
                return document;
            }
    ...
    private bool TryAppendInlineUiContainer(
        ProEdit.FlowDocument.InlineUIContainer container,
        ParagraphBlock paragraph,
        HyperlinkInfo? hyperlink)
    {
