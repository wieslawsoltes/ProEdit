...
public sealed class EditorCommandRouterAdapter : IEditorCommandRouter
{
        ...
        Func<object?, bool>? canExecute = null)
    {
        ...
        Register(commandId, payload => new EditorPayloadCommand(payload, execute), canExecute);
    }

    public bool CanExecute(string commandId, object? payload = null)
    {
        if (string.IsNullOrWhiteSpace(commandId))
        {
            return false;
        ...
        return binding.CanExecute?.Invoke(payload) ?? true;
    }

    public ValueTask<bool> ExecuteAsync(string commandId, object? payload = null)
    {
        if (!CanExecute(commandId, payload))
        {
