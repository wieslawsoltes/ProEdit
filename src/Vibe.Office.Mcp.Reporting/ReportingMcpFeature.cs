...
namespace Vibe.Office.Mcp.Reporting;
...
public sealed class ReportingMcpFeature : IMcpFeature
{
    ...
    private sealed class ReportExportToolHandler : IMcpToolHandler
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var resourceUri = $"vibe://reports/{Uri.EscapeDataString(entry.Id)}/exports/{format.ToString().ToLowerInvariant()}";
    ...
    private sealed class ReportResourceProvider : IMcpResourceProvider
    {
        ...
        public IReadOnlyList<McpResourceDefinition> ListResources(McpRequestContext context)
        {
            ...
            for (var index = 0; index < entries.Count; index++)
            {
                ...
                resources.Add(new McpResourceDefinition
                {
                    ...
                    Description = $"Native VibeOffice JSON definition for report '{entry.Id}'."
                });
            }
        ...
        public ValueTask<McpResourceReadResult?> TryReadAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            if (string.Equals(uri, "vibe://reports/connectors", StringComparison.OrdinalIgnoreCase))
            {
        ...
        switch (format)
        {
            ...
            case ReportExportFormat.Xlsx:
            {
                var profile = new XlsxReportExportProfile
                {
                    ...
                    WorkbookAuthor = GetOptionalString(arguments, "workbookAuthor") ?? "VibeOffice"
                };
                return profile;
            }
    ...
    private static string GetSummaryUri(string reportId) => $"vibe://reports/{Uri.EscapeDataString(reportId)}/summary";

    private static string GetDefinitionUri(string reportId) => $"vibe://reports/{Uri.EscapeDataString(reportId)}/definition";

    private static bool TryParseReportUri(string uri, out string reportId, out string kind)
    {
        reportId = string.Empty;
        kind = string.Empty;
        const string prefix = "vibe://reports/";
