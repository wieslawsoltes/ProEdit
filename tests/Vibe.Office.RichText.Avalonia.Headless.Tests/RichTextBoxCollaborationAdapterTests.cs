...
public sealed class RichTextBoxCollaborationAdapterTests
{
    ...
    public async Task InternalAdapter_AttachSession_AppliesRemoteTextInsert()
    {
        ...
        var flowParagraph = Assert.IsType<Vibe.Office.FlowDocument.Paragraph>(box.Document.Blocks[0]);
        var flowRun = Assert.IsType<Vibe.Office.FlowDocument.Run>(flowParagraph.Inlines[0]);
