...
using Xunit;
using DocumentModel = ProEdit.Documents.Document;
using DocumentParagraphBlock = ProEdit.Documents.ParagraphBlock;
using DocumentRunInline = ProEdit.Documents.RunInline;
...
[assembly: AvaloniaTestApplication(typeof(ProEdit.RichText.Avalonia.Headless.Tests.HeadlessTestAppBuilder))]
...
public sealed class RichTextBoxHeadlessTests
{
    ...
    public async Task EmbeddedUiContainers_Mount_AndRespectDocumentEnabled_Gating()
    {
        ...
        first.Inlines.Add(new ProEdit.FlowDocument.InlineUIContainer
        {
            Child = inlineButton
        });
        document.Blocks.Add(first);
        document.Blocks.Add(new ProEdit.FlowDocument.BlockUIContainer
        {
            Child = blockButton
        });
        for (var i = 0; i < 48; i++)
