...
public sealed class DocxMailMergeTests
{
    ...
    [Theory]
    [InlineData("ADDRESSBLOCK", ProEditDocs.FieldKind.AddressBlock)]
    [InlineData("GREETINGLINE", ProEditDocs.FieldKind.GreetingLine)]
    [InlineData("IF \"City\" = \"Paris\" \"A\" \"B\"", ProEditDocs.FieldKind.MergeRule)]
    [InlineData("NEXT", ProEditDocs.FieldKind.MergeRule)]
    [InlineData("NEXTIF \"City\" = \"Paris\"", ProEditDocs.FieldKind.MergeRule)]
    [InlineData("SKIPIF \"City\" = \"Paris\"", ProEditDocs.FieldKind.MergeRule)]
    public void FieldInstructionParser_ClassifiesAdditionalMailMergeInstructions(string instruction, ProEditDocs.FieldKind expectedKind)
    {
        var definition = ProEditDocs.FieldInstructionParser.Parse(instruction);
    ...
    public void Importer_ClassifiesMergeField_AndPopulatesMailMergeData()
    {
        ...
        try
        {
            ...
            var fieldStart = paragraph.Inlines.OfType<ProEditDocs.FieldStartInline>().First();

            Assert.NotNull(fieldStart.Definition);
            Assert.Equal(ProEditDocs.FieldKind.MergeField, fieldStart.Definition!.Kind);
    ...
    public void Importer_DetectsMailMergeFromAddressBlockField_WithoutMergeSettings()
    {
        ...
        try
        {
            ...
            Assert.NotNull(fieldStart.Definition);
            Assert.Equal(ProEditDocs.FieldKind.AddressBlock, fieldStart.Definition!.Kind);

            Assert.NotNull(document.MailMergeData);
            Assert.Equal(ProEditDocs.MailMergeData.DefaultMainDocumentType, document.MailMergeData!.MainDocumentType);
    ...
    public void Importer_DoesNotDetectMailMergeFromGenericIfFieldWithoutMergeFieldOperand()
    {
        ...
        try
        {
            ...
            var fieldStart = paragraph.Inlines.OfType<ProEditDocs.FieldStartInline>().First();
            Assert.NotNull(fieldStart.Definition);
            Assert.Equal(ProEditDocs.FieldKind.MergeRule, fieldStart.Definition!.Kind);
    ...
    public void Exporter_WritesMailMergeSettings_WhenMailMergeDataPresent()
    {
        ...
        try
        {
            ...
            document.Blocks.Add(new ProEditDocs.ParagraphBlock("Body"));
            document.MailMergeData = new ProEditDocs.MailMergeData();
