...
using ProEdit.Reporting.Rdl;
using ProEdit.Reporting.Serialization;
using ProEdit.Reporting.App.Controls;
using ProEdit.Reporting.App.Services;
using ProEdit.Reporting.App.ViewModels;
using Xunit;

[assembly: AvaloniaTestApplication(typeof(ProEdit.Reporting.App.Headless.Tests.HeadlessTestAppBuilder))]

namespace ProEdit.Reporting.App.Headless.Tests;
...
public sealed class ReportingStudioAppTests
{
    ...
    public async Task MainWindow_RendersStudioShell()
    {
        ...
        Assert.Contains(window.GetVisualDescendants().OfType<TextBlock>(), text => string.Equals(text.Text, "ProEdit Reporting", StringComparison.Ordinal));
        Assert.NotNull(viewModel.ViewerViewModel.CurrentSnapshot);
    ...
    private static string ResolveSampleCorpusPath()
    {
        ...
        {
            if (File.Exists(Path.Combine(directory.FullName, "ProEdit.slnx")))
            {
